...
public void ReserveADCall()
{
        ...
        for (int x = 0; x < board.unitGrid.GetLength(0); x++)
        {
            for (int y = 0; y < board.unitGrid.GetLength(1); y++)
            {
                ...
                buffs.AddRoundBuff(unit, attackSpeedMult: 0f, attackSpeedFlat: 0f, attackDamageFlat: buffAmount, attackDamageMult: 0f, abilityPowerFlat: 0f, abilityPowerMult: 0f, OnHit: null, onHitModifier: 0f);
            }
        }
}
